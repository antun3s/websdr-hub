...
Mesclagem e persistência de status entre execuções, com rastreamento de falhas consecutivas (`consecutive_failures`) e escrita atômica. Carrega o `status.json` anterior (local ou HTTP), mescla com os resultados do health check e republica sem depender de banco de dados.
