...
Interface web single-page que consome a API estática (`stations.json` + `status.json`), mescla dados de catálogo e status, e oferece filtros para navegação. Implementada como HTML+CSS+JS vanilla em arquivo único, zero dependências, servida junto com a API estática no GitHub Pages.
